...
st.write("新レイアウト対応版（上半分：加算登録 ／ 下半分：在庫数量修正）")

# ★吉本さんの本物のGASウェブアプリURLをここに貼り付けてください★
gas_url = "https://google.comhttps://script.google.com/macros/s/AKfycbzqCJKbh31A1MD19mhbLyAhQa2LxN34zs2XxrEaCe64Gl-1uthsF7qzn89fh36J0FH1/exec" 
...
# =============================================================
# 【下半分】ご要望の「在庫数確認 ＆ 直接上書き修正エリア」
# =============================================================
st.subheader("✏️ 2. 現在の在庫状況 確認・直接修正")
...
jan_mod = st.text_input("🔍 在庫を確認・修正するバーコード（JAN）をスキャン：", value="", key=mod_key)

# JANが読み込まれたら即座に現在の在庫を取得して表示
        ...
        if st.button("🚀 この内容で数量を上書き（修正）する", use_container_width=True):
            with st.spinner("クラウド上の在庫データを直接書き換え中..."):
                try:
                    ...
                    st.error(f"🚨 通信エラー: {str(e)}")
    with btn_col2:
        if st.button("❌ 修正入力を消す / キャンセル", use_container_width=True):
            st.session_state.trigger_clear_mod = True
            st.session_state.mod_stock_data = None
            st.session_state.mod_item_name = ""
            st.session_state.last_mod_jan = ""
