function doPost(e) {
  try {
    ...
    for (var r = cValues.length - 1; r >= 0; r--) {
      if (cValues[r][0] !== "") { lastRow = r + 1; break; }
    }
    ...
    // 【バグ修正】全体合計の計算（E列=5 から Q列=17 までの13列分を一括取得。allDataのインデックスは 0=E, 1=F, 2=G... となる）
    ...
    for (var i = 0; i < allData.length; i++) {
      tKatto  += (Number(allData[i][0]) || 0);  // E列 (index 0)
      tWaku   += (Number(allData[i][2]) || 0);  // G列 (index 2)
      tSpace  += (Number(allData[i][4]) || 0);  // I列 (index 4)
      tNakami += (Number(allData[i][6]) || 0);  // K列 (index 6)
      tKanagu += (Number(allData[i][8]) || 0);  // M列 (index 8)
      tShiage += (Number(allData[i][10]) || 0); // O列 (index 10)
      tKanryo += (Number(allData[i][12]) || 0); // Q列 (index 12)
    }
